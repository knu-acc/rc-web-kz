  ...
  return (
    ...
    >
      <nav className="container-custom">
            ...
            {navLinks.map((link) => (
              link.submenu ? (
                <div key={link.label} className="relative group">
                  <button className="relative px-4 py-2 text-secondary-600 font-medium rounded-lg transition-all duration-300 hover:text-primary-600 hover:bg-primary-50 flex items-center gap-1">
                    {link.label}
                    ...
                    <div className="p-2">
                      {link.submenu.map((subLink) => (
                        <Link
                          key={subLink.href}
                          href={subLink.href}
                          className="block px-4 py-2 text-secondary-700 rounded-lg hover:bg-primary-50 hover:text-primary-600 transition-colors"
                        >
                          {subLink.label}
                        </Link>
                      ))}
                ...
                <Link
                  key={link.href}
                  href={link.href}
                  className="relative px-4 py-2 text-secondary-600 font-medium rounded-lg transition-all duration-300 hover:text-primary-600 hover:bg-primary-50 group"
                >
                  {link.label}
        ...
        {/* Mobile Menu */}
        <div
          className={`md:hidden fixed inset-0 top-[72px] bg-white/95 backdrop-blur-xl transition-all duration-500 ${
            isMenuOpen 
              ? 'opacity-100 visible' 
              ...
              {navLinks.map((link, index) => (
                link.submenu ? (
                    ...
                    <div className="pl-4 pb-2 space-y-2">
                      {link.submenu.map((subLink) => (
                        <Link
                          ...
                          href={subLink.href}
                          onClick={closeMenu}
                          className="block text-lg text-secondary-600 py-2 hover:text-primary-600 transition-colors"
                        >
                          {subLink.label}
                        </Link>
                      ))}
                  ...
                  <Link
                    ...
                    href={link.href}
                    onClick={closeMenu}
                    className="text-2xl font-semibold text-secondary-800 py-4 border-b border-secondary-100 transition-colors hover:text-primary-600"
                    style={{ 
