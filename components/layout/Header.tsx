      ...
      >
        ...
        <div className="flex items-center justify-between">
              ...
              if (link.submenu) {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`absolute top-full left-1/2 -translate-x-1/2 mt-1 w-52 bg-white dark:bg-secondary-900 rounded-xl overflow-hidden border border-secondary-100 dark:border-secondary-800 transition-all duration-200 z-50 ${
                        isSubmenuOpen ? 'opacity-100 visible shadow-[0_4px_20px_rgba(0,0,0,0.08)]' : 'opacity-0 invisible shadow-none'
                      }`}
                      role="menu"
                    ...
                    >
                        {link.submenu.map((subLink) => (
                          <Link
                            key={subLink.href}
                            href={subLink.href}
                            className="block px-4 py-2.5 text-sm text-secondary-600 dark:text-secondary-300 hover:bg-secondary-50 dark:hover:bg-secondary-800 hover:text-secondary-900 dark:hover:text-white transition-colors"
                            role="menuitem"
          ...
          <div className="hidden md:flex items-center gap-3">
            <ThemeToggle />
            <WhatsAppLink
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-white rounded-lg bg-primary-600 hover:bg-primary-700 transition-colors"
            ...
            {navLinks.map((link, index) => (
              link.submenu ? (
                <div key={link.label} className="border-b border-secondary-100 dark:border-secondary-800">
                  <button
                    className="w-full text-left text-base font-semibold text-secondary-800 dark:text-secondary-100 py-3 flex items-center justify-between"
                    onClick={() => setOpenSubmenu(openSubmenu === link.label ? null : link.label)}
                    aria-expanded={openSubmenu === link.label}
                    aria-controls={`mobile-submenu-${index}`}
                  >
                    <span>{link.label}</span>
                    <svg className={`w-4 h-4 transition-transform ${openSubmenu === link.label ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" />
                    </svg>
                  </button>
                  <div
                    id={`mobile-submenu-${index}`}
                    className={`pl-4 pb-1 space-y-1 overflow-hidden transition-all duration-500 ease-in-out ${
                  ...
                  >
                    {link.submenu.map((subLink) => (
                      <Link
                        ...
                        href={subLink.href}
                        onClick={closeMenu}
                        className="block text-sm text-secondary-600 dark:text-secondary-300 py-2 hover:text-secondary-800 dark:hover:text-white transition-colors"
                      >
                        {subLink.label}
                      </Link>
                    ))}
                ...
                <Link
                  ...
                  href={link.href}
                  onClick={closeMenu}
                  className={`text-base font-semibold py-3 border-b border-secondary-100 dark:border-secondary-800 transition-colors ${
                    pathname === link.href
                      ? 'text-primary-600 dark:text-primary-400'
