...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const parseMarkdown = (text: string) => {
    ...
    // Функция для генерации уникального ID заголовка
    ...
    const flushUl = () => {
      ...
      out.push(
        <ul key={uid++} className="my-4 rounded-2xl bg-secondary-50/80 dark:bg-secondary-900/50 border border-secondary-200/80 dark:border-secondary-800/70 p-5 sm:p-6 space-y-2 list-disc pl-6 marker:text-primary-500 dark:marker:text-primary-400">
          {buf.map((item, idx) => (
            <li key={`li-${uid}-${idx}`} className="text-[1.05rem] leading-[1.7] text-secondary-700 dark:text-secondary-300">
              {inline(item, uid + idx)}
            </li>
          ))}
        </ul>
          ...
          <table className="min-w-full text-[0.95rem]">
            <thead>
              <tr className="bg-primary-50 dark:bg-primary-900/20">
                {headers.map((h, hi) => (
                  <th key={`th-${uid}-${hi}`} className="px-6 py-4 text-left font-semibold text-secondary-900 dark:text-white">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-secondary-100 dark:divide-secondary-800">
              {rows.map((r, ri) => (
                <tr key={`tr-${uid}-${ri}`} className="bg-white dark:bg-secondary-950 even:bg-secondary-50/50 dark:even:bg-secondary-900/30">
                  {r.map((c, ci) => (
                    <td key={`td-${uid}-${ri}-${ci}`} className="px-6 py-4 text-secondary-700 dark:text-secondary-300 leading-relaxed">{inline(c, uid + ri)}</td>
                  ))}
                </tr>
              ))}
            </tbody>
    ...
    lines.forEach((line) => {
      ...
      if (t.startsWith('# ')) {
        flush()
        const headingText = t.slice(2).trim()
        const headingId = generateUniqueHeadingId(headingText)
        out.push(
          <h2 key={uid++} id={headingId}
            className="mt-10 mb-5 text-[1.65rem] sm:text-[1.9rem] font-extrabold leading-tight text-secondary-900 dark:text-white">
            {inline(headingText, uid)}
          </h2>
        )
        return
      }
      if (t.startsWith('## ')) {
        flush()
        const headingText = t.slice(3).trim()
        const headingId = generateUniqueHeadingId(headingText)
        out.push(
          <div key={uid++} className="mt-10 mb-5">
            <h2 id={headingId} className="text-[1.35rem] sm:text-[1.5rem] font-bold leading-snug text-secondary-900 dark:text-white pb-3 border-b-2 border-primary-500/30 dark:border-primary-400/30">
              {inline(headingText, uid)}
            </h2>
          </div>
        )
      ...
      if (t.startsWith('### ')) {
        flush()
        const headingText = t.slice(4).trim()
        const headingId = generateUniqueHeadingId(headingText)
        out.push(
          <h3 key={uid++} id={headingId}
  ...
  return (
    <>
      ...
      <article className="bg-white dark:bg-secondary-950">
        ...
        {/* ─── ТЕЛО СТАТЬИ ─── */}
        <div className="container-custom pt-8 sm:pt-12 pb-20 lg:pb-28">
            ...
            <div className="max-w-3xl">
              ...
              <div className="mt-16 p-8 sm:p-10 rounded-2xl bg-gradient-to-br from-primary-50 to-primary-100/50 dark:from-primary-950/60 dark:to-primary-900/40 border border-primary-200/60 dark:border-primary-700/60">
                ...
                <div className="flex flex-wrap gap-4">
                  <Link href="/contact" className="inline-flex items-center gap-2.5 px-7 py-3.5 bg-primary-600 hover:bg-primary-700 dark:bg-primary-500 dark:hover:bg-primary-600 text-white font-semibold rounded-xl transition-colors text-[0.95rem] min-h-[48px] touch-manipulation">
                  ...
                  <div className="grid sm:grid-cols-2 gap-5">
                    {relatedPosts.slice(0, 2).map((rp) => (
                      <Link key={rp.slug} href={`/blog/${rp.slug}`}
                        className="group flex flex-col p-6 sm:p-7 rounded-2xl border border-secondary-200 dark:border-secondary-800 hover:border-primary-300 dark:hover:border-primary-700 bg-white dark:bg-secondary-900/50 hover:shadow-lg transition-all touch-manipulation">
