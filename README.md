...
### 4. Сборка для production (статический экспорт)

```bash
npm run build
```

Собранные файлы будут в папке `out/` (статический экспорт для обычного хостинга)

**Важно:** После сборки скопируйте файл `public/.htaccess` в папку `out/` для правильной работы на Apache сервере.
